...
sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz-${HEADS}\"|g" "arch/arm64/configs/surya_deconfig"
...
if ! [ -d "${CLANG_DIR}" ]; then
  ...
  if ! git clone --depth=1 https://gitlab.com/XSans0/weebx-clang/-/tree/release/15-gr ${TC_DIR}; then
