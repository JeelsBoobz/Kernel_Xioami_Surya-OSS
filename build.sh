...
FINAL_KERNEL_ZIP=MiHomo-Surya-$(date '+%Y%m%d').zip
...
rm -rf weebx-clang && mkdir weebx-clang && tar -xvf weebx-clang.tar.gz -C weebx-clang && rm -rf weebx-clang.tar.gz
exit 1
fi
sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-MiHomo\"|g" arch/arm64/configs/$KERNEL_DEFCONFIG
