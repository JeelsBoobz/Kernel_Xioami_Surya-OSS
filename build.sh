...
LOCAL_DIR=$HOME
ZIPNAME="JeelsBoobzKernel-Surya-$(TZ=Asia/Jakarta date +"%Y%m%d-%H%M").zip"
ZIPNAME_KSU="JeelsBoobzKernel-Surya-KSU-$(TZ=Asia/Jakarta date +"%Y%m%d-%H%M").zip"
...
export KBUILD_BUILD_USER="BangJeels"
export KBUILD_BUILD_HOST="JeelsBoobz"
...
if [[ $1 = "-k" || $1 = "--ksu" ]]; then
    ...
    sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz-KSU\"|g" arch/arm64/configs/vendor/$DEFCONFIG
else
    echo -e "\nKSU not Support, let's Make it off\n"
    sed -i 's/CONFIG_KSU=y/CONFIG_KSU=n/g' arch/arm64/configs/vendor/$DEFCONFIG
    sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz\"|g" arch/arm64/configs/vendor/$DEFCONFIG
fi
...
git restore arch/arm64/configs/vendor/$DEFCONFIG
if [ -d "$AK3_DIR" ]; then
