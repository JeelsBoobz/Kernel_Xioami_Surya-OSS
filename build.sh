...
export HEADS=$(git rev-parse --short HEAD)
[[ ! -z "$KSU" && -f "arch/arm64/configs/surya_defconfig" ]] || exit 1
SECONDS=0 # builtin bash timer
if [ "${KSU}" = "true" ]; then
  ZIPNAME="JeelsBoobz-KSU-${HEADS}-$(date +"%Y%m%d").zip"
  sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz-KSU${HEADS}\"|g" "arch/arm64/configs/surya_defconfig"
else
  ZIPNAME="JeelsBoobz-${HEADS}-$(date +"%Y%m%d").zip"
  sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz-${HEADS}\"|g" "arch/arm64/configs/surya_defconfig"
...
if [ -f "out/arch/arm64/boot/Image.gz-dtb" ] && [ -f "out/arch/arm64/boot/dtbo.img" ]; then
  ...
  sed -i "s|kernel.string=.*|kernel.string=JeelsBoobz Kernel|g" ./anykernel.sh
