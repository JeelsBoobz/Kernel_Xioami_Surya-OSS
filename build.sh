...
if [ "${KSU}" = "true" ]; then
  ZIPNAME="JeelsBoobz-KSU-$(date +"%Y%m%d").zip"
  sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz-KSU\"|g" "arch/arm64/configs/surya_defconfig"
else
  ZIPNAME="JeelsBoobz-$(date +"%Y%m%d").zip"
  sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-JeelsBoobz\"|g" "arch/arm64/configs/surya_defconfig"
